		...
		<div class="ui-main-form" style="background:#fff;">
			<div class="ui-main-form-header" style="background:#79B6D8;">
				<table align="left" height="40px" border="0">
					<tr>
						<td><span style="margin-left:10px;">MRIN Approval View</span></td>
						<!--<?php if ($this->input->get('pr') == 'pending') { echo 'PR';}else{ echo 'MIRN' ;} ?>-->
					</tr>
				</table>
			</div>
				...
				<div class="middle_d">
					<!-- <table width="100%" class="ui-content-form-reg" style="">
						<tr >
							<td class="ui-desk-style-table">
								<table class="ui-content-form" width="100%" border="0">
									<tr>
										<td colspan="2" class="ui-bottom-border-color" style="font-weight: bold;">
											Item Specification <?php if ($this->input->get('pro') == 'pending' && $classid == 3) { ?>
											<a href="<?php echo base_url();?>index.php/Procurement?mrinno=<?=$this->input->get('mrinno')?>&pro=edit" class="btn-button btn-primary-button" style="width: 50px; display:inline-block; margin-bottom:5px; font-weight:bold; text-align:center;" name="mysubmit" value="Edit">Edit</a>
											<?php } ?>
										</td>
									</tr>
									<tr>
										...
										<td>
											...
											<table class="wnctable" border="1" style="text-align:center;" align="center">
												...
												<?php if($this->input->get('pro') == 'pending' && ($classid == 1 || $classid == 3)){?>
													<th>Qty Approved</th>
													<?php // if (($this->input->get('pr') == 'approved') or ($this->input->get('pr') == 'pending')){ echo 'AM'; }?>
												<?php } ?>
												<?php if ($this->input->get('pro') == 'pending' && $classid == 3) { ?>
													<th>Price</th>
					...
					<style type="text/css">
						...
						/*@media screen and (min-width: 375px) and (max-width: 425px){
							.p_itemspec{width: 100%; display: table;}
							.p_wnclabel{width: 10%; float: left; padding: 10px;}
							.p_wnctable{width: 79%; float: left; padding: 10px;}
						}
						@media screen and (min-width: 0px) and (max-width: 320px){
							.p_itemspec{width: 100%; display: table;}
							.p_wnclabel{width: 10%; float: left; padding: 10px;}
							.p_wnctable{width: 75%; float: left; padding: 10px;}
						}*/
					</style>
					<div class="ui-content-form-reg">
						<div style="color:white; width: 100%;" height="30px">
							<div class="ui-header-new">
								<b>
									Item Specification
									<?php if ($this->input->get('pro') == 'pending' && $classid == 3) { ?>
										<a href="<?php echo base_url();?>index.php/Procurement?mrinno=<?=$this->input->get('mrinno')?>&pro=edit" class="btn-button btn-primary-button" style="width: 50px; display:inline-block; margin-bottom:5px; font-weight:bold; text-align:center;" name="mysubmit" value="Edit">Edit</a>
									...
									<div class="p_itemspec" style="width: 100%; display: inline-block;">
										...
										<div class="p_wnctable">
											<div style="width: auto; overflow-x: auto;">
												<?php if ($itemrec) { ?>
												<table class="wnctable" border="1" style="text-align:center;" width="100%" align="center">
													<tr>
														<th>No</th>
														<th>Item Code</th>
														<th>Item Name</th>
													...
													<tr>
														<td><?=$numrow++?></td>
														<td><b><?=isset($row->ItemCode) ? $row->ItemCode : ''?></b></td>
														<td><b><?=isset($row->ItemName) ? $row->ItemName : ''?></br><?php if(!empty($row->RN_No))echo '(RN No: '. $row->RN_No .')'; if(!empty($row->QtyRN)) echo '</br>(Qty: '. $row->QtyRN .')';?></b></td>
														<?php if($this->input->get('pro') == 'pending' && $classid == 1){?>
														<td><b><?=isset($row->Unit_Cost) ? number_format($row->Unit_Cost,2) : ''?></b></td>
														<?php } ?>
														<td><b><?=isset($row->Qty) ? $row->Qty : ''?></b></td>
										...
										<td>
											<table class="wnctable" border="1" style="text-align:center; border:1px solid black;" align="center">
												...
												<tr>
													...
													<td><b><?=isset($row->QtyReqfx) ? $row->QtyReqfx : ''?></b></td>
													<td><b><?=isset($row->Unit_Costx) ? number_format(($row->Unit_Costx * $row->QtyReqfx),2) : '0.00'?></b></td>
													<?php
														$total += $row->Unit_Costx * $row->QtyReqfx;
													?>
												</tr>
												<?php endforeach; ?>
												<tr>
