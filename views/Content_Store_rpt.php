...
<?php  ?>
	...
	<table class="tftable" border="1" style="text-align:center;width:95%" align="center">
		...
		<tr>
			<td><?= $num?></td>
			<td><?= isset($row->RN_No) ? $row->RN_No : '' ?></td>
			<td><?= isset($row->Date_Stamp) ? $row->Date_Stamp : '' ?></td>
			<td><?= isset($row->MIRNcode) ? $row->MIRNcode : '' ?></td>
			<td><?= isset($row->WorkOfOrder) ? $row->WorkOfOrder : '' ?></td>
			<td><?= isset($row->ItemCode) ? $row->ItemCode : '' ?></td>
			<td><?= isset($row->ItemName) ? $row->ItemName : '' ?></td>
			<td><?= isset($row->relqty) ? $row->relqty : '' ?></td>
			<td><?= isset($row->relqty) ? $row->relqty*$row->Price_Taken : '' ?></td>
		</tr>
		<?php $num = $num+1;
		endforeach; ?>
<!-- </div> -->
