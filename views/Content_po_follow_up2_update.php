...
<td colspan='4'  id="spcomponent1"><?php foreach($pocom as $row){
	if (isset($row->component_name)){
			...
			echo "<span class='icon-play icon' style='font-size:15px;'></span><span style='font-size:15px; font-weight:bold;'>" .$row->component_name. "</span><a href=".base_url()."uploadpofiles/".$row->com_id."><span class='icon-file-text2 icon'></a></span><a href='javascript:fCallLocatiod(\"".$row->PO_No."\",\"".$row->Id."\",\"component\",\"1\");'><span class='icon-cross icon' style='color:red;'></span></a><a href='javascript:fCallLocatioe(\"".$row->PO_No."\",\"".$row->Id."\",\"component\",\"1\");'><span class='icon-new icon'></span></a>";
			echo '<br>';
			} else {
			echo "<span class='icon-play icon' style='font-size:15px;'></span><span style='font-size:15px; font-weight:bold;'>" .$row->component_name. "</span> <br/><a href=".base_url()."uploadpofiles/".$row->com_id."><img src=".base_url()."uploadpofiles/".$row->com_id." style='width:50px; height:50px; padding-left:5px;' ></a><a href='javascript:fCallLocatiod(\"".$row->PO_No."\",\"".$row->Id."\",\"component\",\"1\");'><span class='icon-cross icon' style='color:red;'></span></a><a href='javascript:fCallLocatioe(\"".$row->PO_No."\",\"".$row->Id."\",\"component\",\"1\");'><span class='icon-new icon'></span></a>";
			echo '<br>';
			}}
		} ?></td>
</tr>
...
<tr><td colspan="3" class="ui-bottom-border-color" ><?php if($no_pay==1){echo '1st';}elseif($no_pay==2){echo'2nd';}else{ echo '3rd';} echo ' Payment'; ?></td></tr>
		<tr style="height:20px;">
			...
			<tr>
				<td class="td-assest">Payment Amount</td>
				<td><input type="text"  name="n_payamt<?=$no_pay?>" id="n_payamt<?=$no_pay?>" value="<?php echo ($payment_amount[$no_pay]!='')?number_format($payment_amount[$no_pay],2, '.', ''):$this->input->post('n_payamt'.$no_pay)?>" class="form-control-button2 n_wi-date2" ></td>
			</tr>
			<tr>
				<td class="td-assest">Payment Balance :</td>
				<td><input type="text"  name="n_paybal<?=$no_pay?>" id="n_paybal<?=$no_pay?>" value="<?=number_format($payment_balance[$no_pay],2, '.', '')?>" class="form-control-button2 n_wi-date2" ></td>
			</tr>
...
<td colspan='4'  id="spattachment<?=$no_pay?>"><?php foreach($pocat as $row){
	if (isset($row->Doc_name)&&$row->visit==$no_pay){
			...
			echo "<span class='icon-play icon' style='font-size:15px;'></span><span style='font-size:15px; font-weight:bold;'>" .$row->Doc_name. "</span><a href=".base_url()."uploadfinfiles/".$row->doc_id."><span class='icon-file-text2 icon'></a></span><a href='javascript:fCallLocatiod(\"".$row->PO_No."\",\"".$row->Id."\",\"attachment\",\"".$no_pay."\");'><span class='icon-cross icon' style='color:red;'></span></a><a href='javascript:fCallLocatioe(\"".$row->PO_No."\",\"".$row->Id."\",\"attachment\",\"".$no_pay."\");'><span class='icon-new icon'></span></a>";
			echo '<br>';
			} else {
...
function fCallLocatioa(pono,tag,payment_no)
	...
	function fCallLocatioe(pono, id, tag, payment_no)
	{
		var url		=	'<?php echo base_url ('index.php/Procurement/asset3_comm_newpo') ?>?pono='+pono+'&id='+id+'&act=update'+'&tag='+tag+'&payment_no='+payment_no;
	...
	function fCallLocatiod(pono, id, tag, payment_no)
	{
		var url		=	'<?php echo base_url ('index.php/Procurement/asset3_comm_newpo') ?>?pono='+pono+'&id='+id+'&act=delete'+'&tag='+tag+'&payment_no='+payment_no;
 		// document.getElementById('spcomma'+tag+payment_no).innerHTML = '';
		// $('#spcomma'+tag+payment_no).load(url);
 		document.getElementById('spcomma'+tag+payment_no).style.display='none';
		document.body.style.cursor='default';
		winProp = 'width=450,height=270,left=' + ((screen.width - 600) / 2) +',top=' + ((screen.height - 400) / 2) + ',menubar=no, directories=no, location=no, scrollbars=yes, statusbar=no, toolbar=no, resizable=no';
		Win = window.open(url, 'Location', winProp);
		Win.window.focus();
		document.getElementById('spcomma'+tag+payment_no).style.display='block';
