...
class Ppm_genstd_ctrl extends CI_Controller {
  ...
  { //echo "ini first";
		...
		$nilaiact = $_GET["act"];
		$nilaim = $_GET["m"];
	  	$nilaiasset = $_GET["assetno"];
	  	$nilaijt = $_GET["jobtype"];
	  	//echo $nilaijt.'<br><br>';
	  ...
	  $this->load->model('get_model');	
		...
		if ($value->v_Asset_no == $nilaiasset AND $value->v_JobType == $nilaijt){
		$this->load->model('get_model');
    ...
    //$value['ppm_no']=$sFormatP;
		...
		redirect('ppm_gen_ctrl?wk='.$nilaiwk.'&y='.$nilaiy.'&d='.$nilaid.'&m='.$nilaim.'&act=');
		/*else{
		//echo "x masuk act";
		$data['y']= ($this->input->get('y') <> 0) ? $this->input->get('y') : 'NA';	
		$data['wk']= ($this->input->get('wk') <> 0) ?  $this->input->get('wk') : 'NA';
		...
		foreach ($data['ppm_wo'] as $value) {
		//$this->load->model('get_model');
		$truker = $this->get_model->validate_schmon($nilaiwk, $this->session->userdata('hosp_code'), $nilaiy, $value->v_Asset_no);
		if (count($truker) == 0) {
