...
class Vendor_reg_ctrl extends CI_Controller{
	...
	if ($this->input->post('n_vendor_code') == ''){
		...
		$nextvencode = $this->get_model->vendorcodegenx();
		//$vendor_code = strtoupper($venname).str_pad($nextvencode[0]->N_vencode,$nextvencode[0]->T_vencode,0,STR_PAD_LEFT);
		$vendor_code = "APSB_VEN_".str_pad($nextvencode[0]->N_vencode,$nextvencode[0]->T_vencode,0,STR_PAD_LEFT);
		...
		$insert_data2 = array('EQUIPMENT_TYPE_NAME' => $this->input->post('n_vendor_itemname'),
							 //'VENDOR_CODE' => $nextvencode[0]->T_vencode,$vendor_code
							 'VENDOR_CODE' => $vendor_code,
							 'VENDOR_NAME' => $this->input->post('n_vendor_name'),
							 'ADDRESS' => $this->input->post('n_Address'),
							 'ADDRESS2' => $this->input->post('n_Address_2'),
							 'ADDRESS3' => $this->input->post('n_Address_3'),
							 'TELEPHONE_NO' => $this->input->post('n_phone_nombor'),
							 'FAX_NO' => $this->input->post('n_fax_nombor'),
							 'CONTACT_PERSON' => $this->input->post('n_contact'),
							 //'POSITION' => $this->input->post('n_vendor_tel'),
							 'CONTRACTOR_SUPPLIER' => $this->input->post('n_vendor_type'),
							 'BUMI_STATUS' => $this->input->post('n_company_type'));
