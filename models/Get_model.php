...
$this->db->where('v_Actionflag <> ', 'D');
$this->db->where('v_check_code = ', $chklistcd);
//$this->db->where('v_check_code = ', 'CFM0142-01');
$query = $this->db->get();
$heppm = array();
$heppm[] = new stdClass();
$heppm[0]->recID = null;
$heppm[0]->assetType = null;
$heppm[0]->assetTypeDescription = null;
$heppm[0]->mfg = null;
$heppm[0]->model = null;
$heppm[0]->freq = null;
$heppm[0]->freqname = null;
$heppm[0]->KKMRefNo = null;
$heppm[0]->checklistCode = null;
$heppm[0]->checklistDescription = null;
$heppm[0]->checklistVersion = null;
$heppm[0]->effectiveDate = null;
$heppm[0]->volumeNo = null;
$heppm[0]->checklistFilename = null;
$heppm[0]->actionFlag = null;
$heppm[0]->actionTimestamp = null;

foreach ($query->result() as $key => $row) {
$heppm[0]->freq = $row->v_Intervals;
$heppm[0]->checklistCode = $row->v_check_code;
$heppm[0]->checklistDescription = $row->v_checkDesc;
}
return $heppm;
}
//echo "laalla".$query->checklistDescription;
/* echo $this->db->last_query();
exit(); */
...
function vendorcodegen($venname){
	$this->db->select('right(v_vendorcode,length(v_vendorcode) - 3) + 1 as N_vencode, length(v_vendorcode) - 9 as T_vencode',FALSE);
...
function nextponumber($mrinno=''){
	...
	$aponie1 = str_replace("NJ","JHR",$aponie);
	$aponie1 = str_replace("SJ","JHR",$aponie1);
	$aponie1 = str_replace("N9","NSN",$aponie1);
	$aponie1 = str_replace("KA/","MKA/",$aponie1);
	if ($area2=="MK") {
		$aponie1 = str_replace("MG/","MKA/",$aponie1);
	} elseif (($area2=="NJ") || ($area2=="SJ")) {
		$aponie1 = str_replace("MG/","JHR/",$aponie1);
	} elseif ($area2=="N9") {
		$aponie1 = str_replace("MG/","NSN/",$aponie1);
	}
	//$aponie1 = str_replace("MG/","",$aponie1);
	...
	if ($area == "IMG") {
		$this->db->select("CONCAT('PO/CHO/02/3/','".$aponie1."/".$alphabet[$monthe].date('Y')."','/',RIGHT(CONCAT('0000',CAST(po_next_no AS char)), 4)) AS pono,po_next_no",FALSE);
	} else {
		$this->db->select("CONCAT('PO/OPU-02/','".$aponie1."/".$alphabet[$monthe].date('Y')."','/',RIGHT(CONCAT('0000',CAST(po_next_no AS char)), 4)) AS pono,po_next_no",FALSE);
	}
...
$query = $this->db->select("*")->from("tbl_rn_autono")->order_by("rn_next_no", "desc")->limit(1)->get()->result();
$number = str_pad($next_number, 5, '0', STR_PAD_LEFT);//$this->db->where_in('user_id', $users)
$listsite = array('N9','MKA','NJ','SJ');
$query = $this->db->select("Zone_Code")->from("tbl_zone_hosp")->where_in('zone_code', $listsite)->where('Hosp_Code', $to)->get()->result();
$from = $query[0]->Zone_Code;
switch ($to) {
		case "JLB":
		case "JMP":
		case "KPL":
	  ...
	  $sepital =($this->input->get('id')) ? ($this->input->get('id')) : $this->session->userdata('hosp_code');
		  $this->db->distinct();
		   ...
		   $this->db->limit(10000);
			...
			$this->db->group_start();
			$this->db->or_like("b.ItemCode",$searchitem)->or_like("b.ItemName",$searchitem);
			$this->db->group_end();
			}
		...
		public function check_mirncomp($mirncd="",$itemcd="") {
		     $this->db->select('ItemCode');
		     //$this->db->where('userid ',$this->session->userdata('v_UserName'));
				 $this->db->where('MIRNcode', $mirncd);
		     $this->db->where('ItemCode', $itemcd);
		     $query = $this->db->get('tbl_mirn_comp');
		     $data = $query->row();
				 //echo $this->db->last_query();
		 		//exit();
		     if($query->num_rows() > 0) {
		...
		function getmrinstat($month, $year)
		{
		$this->db->select("count(DISTINCT(a.DocReferenceNo)) as total_mrin, SUM(CASE WHEN a.ApprStatusIDxx <> '4' THEN 1 ELSE 0 END) as pending, count(DISTINCT(b.MIRN_No)) as poed");
		$this->db->from('tbl_materialreq a');
		$this->db->join('tbl_po_mirn b','a.DocReferenceNo = b.MIRN_No','left');
		//$this->db->where('UserId','APSB039');
		...
		function vendorcodegenx(){
			$this->db->select('right(VENDOR_CODE,4) + 1 as N_vencode, length(VENDOR_CODE) - 3 as T_vencode',FALSE);
