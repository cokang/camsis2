...
//$this->db->select('pmis2_sa_asset_mapping.new_asset_type, left(pmis2_sa_moh_asset_type.type_desc, 50)');
//$this->db->select(" ifnull(max(MID(V_Tag_no,5,8)),0) + 1 AS thenum ", FALSE);
$this->db->select(" ifnull(max(MID(V_Tag_no,6,1)),0) AS nm2 ", FALSE);
$this->db->where("V_service_code = ", $this->session->userdata('usersess'));
...
$this->db->join('pmis2_egm_assetjobtype job','ag.v_hospital_code = job.v_hospitalcode AND ar.v_asset_no = job.v_asset_no ');
$this->db->where('ppm.v_Actionflag <> ', 'D');
...
function locdjic($month,$year,$hosp,$loc){
	$this->db->select('hospital_code,ji_no,Dept_Code,Loc_Code,Job_Date,SUM(CASE when (Num_Code = "1" OR Num_Code = "5") AND Job_Items = "Floor" then 1 else 0 end) AS Floor,SUM(CASE when (Num_Code = "1" OR Num_Code = "5") AND Job_Items = "Wall Door" then 1 else 0 end) AS WallDoor,SUM(CASE when (Num_Code = "1" OR Num_Code = "5") AND Job_Items = "Ceiling" then 1 else 0 end) AS Ceiling,
					  SUM(CASE when (Num_Code = "1" OR Num_Code = "5") AND Job_Items = "Windows" then 1 else 0 end) AS Windows,SUM(CASE when (Num_Code = "1" OR Num_Code = "5") AND Job_Items = "Fixtures" then 1 else 0 end) AS Fixtures,SUM(CASE when (Num_Code = "1" OR Num_Code = "5") AND Job_Items = "Furniture Fitting" then 1 else 0 end) AS FurnitureFitting');
	$this->db->from('set_jic_scheduler');
	//$this->db->where('Month',$month);
	//$this->db->where('Year',$year);
	$this->db->where('Job_date >=', $this->dater(1,$month,$year));
