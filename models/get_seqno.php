...
//===BERAPO KOSONG NAK LOTAK KEK DOPAN? LIMO KOSONG? ONAM KOSONG? SERI SATU SAMO======
	...
	switch ($sType) {
		...
		case "A11" :
		case "A12" :
			$sFormat = str_replace("DDD", "AV", $sFormat); //nak tukau apa2 cari mangkuk ni!
			break;
		...
		break;
		case "AP1" :
			$sFormat = str_replace("DDD", "AP1", $sFormat);
			break;
			case "AP19" :
			$sFormat = str_replace("DDD", "AP19", $sFormat);
	}
	...
	{
	$sFormat = str_replace("BBB", substr($this->session->userdata('usersess'),-4, 1), $sFormat);
	}else {
